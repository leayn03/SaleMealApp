...
public class HistoryDetailOrder extends BaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.historyorderdetail);
		...
		detailListView=(ListView)findViewById(R.id.orderHistoryList);
		
		 initOrderDetailList();


		detailReturnButton=(ImageButton)findViewById(R.id.detailHistoryReturnButton);
...
//				startActivity(intent);
				HistoryDetailOrder.this.finish();
			}
		});
	...
	private void initOrderDetailList()
	{
		...
		setUIRefreshConfig(new UIThreadImpl() {
			...
			public void refresh(Message msg) {
				...
				if(0==msg.arg2)
				{
					detailListView.setAdapter(new MyAdapter(HistoryDetailOrder.this,R.layout.historyorderdetail_row, orderDetailList, 
							new String[]{"saleUserName",  "saeUserTotal"}, new int[]{R.id.goodSaleHistoryUserOid, R.id.goodSaleHistoryUserTotal}));	
					loginProgressDialog.dismiss();  //progressbar 4
				}
				if(1==msg.arg2)
				{
					loginProgressDialog.dismiss();  //progressbar 4
					Toast.makeText(HistoryDetailOrder.this, "订单已损坏！", Toast.LENGTH_LONG).show();
				}

			}
		});
		
		setAsynThreadConfig("getOrderHistoryDetailList", true, new AsynThreadImpl() {
			...
			public Message excute() {
				...
				finishAsynThread("getOrderHistoryDetailList");
				
				return msg;
			}
		});
		startAsynThread("getOrderHistoryDetailList"); 
	}
